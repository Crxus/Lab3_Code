...
_fifo_map_swappable(struct mm_struct *mm, uintptr_t addr, struct Page *page, int swap_in)
{
    ...
    //ADD PART: LAB2 PRA2
    list_add(head,entry);    //To add the recently used page into the pra list, this page is the most recently used page(That means it will be swapped out first
    return 0;
}
/*
...
_fifo_swap_out_victim(struct mm_struct *mm, struct Page ** ptr_page, int in_tick)
{
     ...
     /*LAB3 EXERCISE 2: YOUR CODE*/ 
     // unfinished
     list_entry_t *le= head->prev;  //now we guess the head of pra does not save anything, its first not null index is head->next
     //assert(le!=NULL);
     assert(le!=head);
